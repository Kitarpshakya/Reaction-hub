...
export default function Header() {
  ...
  const isPeriodicTablePage = pathname === "/periodic-table"
  const isCompoundsPage = pathname?.startsWith("/compounds")
    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-[#0F0F1E]/80 backdrop-blur-md border-b border-white/10">
      <div className="container mx-auto px-4 py-2">
        <div className="flex items-center justify-between">
          ...
          {/* Navigation - Conditional based on current page */}
          <nav className="hidden md:flex items-center gap-6">
            {/* Show Periodic Table link only on Compounds page */}
            {isCompoundsPage && (
              <Link
                href="/periodic-table"
            ...
            {/* Show Compounds link only on Periodic Table page */}
            {isPeriodicTablePage && (
              <Link
                href="/compounds"
                className="text-white/80 hover:text-white transition-colors"
            ...
            {/* Don't show any navigation links on Home page */}
          </nav>
